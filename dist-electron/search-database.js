...
export function assignPersonToFace(faceId, personId) {
    const database = getDb();
...
export function getAiStats() {
    const database = getDb();
    const processed = database.prepare(`SELECT COUNT(*) as cnt FROM ai_processing_status WHERE face_processed = 1 OR tags_processed = 1`).get().cnt;
    const faces = database.prepare(`SELECT COUNT(*) as cnt FROM face_detections`).get().cnt;
    const persons = database.prepare(`SELECT COUNT(*) as cnt FROM persons`).get().cnt;
    const tags = database.prepare(`SELECT COUNT(DISTINCT tag) as cnt FROM ai_tags`).get().cnt;
    const totalPhotos = database.prepare(`SELECT COUNT(*) as cnt FROM indexed_files f INNER JOIN indexed_runs r ON f.run_id = r.id WHERE f.file_type = 'photo'`).get().cnt;
