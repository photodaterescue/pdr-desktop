...
const BACKGROUND_MIRROR_INTERVAL_MS = 30000;
let dbDirty = false;
let mirrorIntervalHandle = null;
let mirrorInFlight = false;
let cachedDeviceName = 'Unknown';
export function markDbDirty() {
    dbDirty = true;
}
export function setBackgroundMirrorDeviceName(name) {
    if (name && name.trim())
        cachedDeviceName = name.trim();
}
export function startBackgroundMirror() {
...
export function stopBackgroundMirror() {
    if (mirrorIntervalHandle) {
        clearInterval(mirrorIntervalHandle);
        mirrorIntervalHandle = null;
    }
}
async function tickBackgroundMirror() {
    if (!dbDirty)
        return;
    if (mirrorInFlight)
        return;
    ...
    try {
        ...
        dbDirty = true;
        console.warn('[LibSidecar] background mirror threw:', e.message);
    }
    finally {
