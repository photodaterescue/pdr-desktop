...
export interface GeoResult {
  country: string;       // Full country name e.g. "Thailand"
  countryCode: string;   // ISO 2-letter e.g. "TH"
  city: string;          // City name e.g. "Bangkok"
  admin1: string;        // State/province code
  distance: number;      // Distance in km from the GPS point
}
...
interface KDNode {
  ...
  left: KDNode | null;
  right: KDNode | null;
  splitDim: number; // 0 = lat, 1 = lon
}

function buildKDTree(points: CityRecord[], depth: number = 0): KDNode | null {
...
function nearestNeighbor(root: KDNode | null, targetLat: number, targetLon: number, best: NNBest): void {
  if (!root) return;
 ...
 */
export function reverseGeocode(lat: number, lon: number, maxDistanceKm: number = 200): GeoResult | null {
  if (!kdTree || !loaded) return null;
  if (lat < -90 || lat > 90 || lon < -180 || lon > 180) return null;
  if (isNaN(lat) || isNaN(lon)) return null;

  const best: NNBest = { node: null, dist: Infinity };
  nearestNeighbor(kdTree, lat, lon, best);

  if (!best.node || best.dist > maxDistanceKm) return null;
