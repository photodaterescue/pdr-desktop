 ...
 * cleanly.
 */
async function lsFetchJson(url, body, timeoutMs) {
...
async function validateWithLemonSqueezy(licenseKey, instanceId) {
    ...
    try {
        const data = await lsFetchJson(`${LEMON_SQUEEZY_API}/validate`, { license_key: licenseKey, instance_id: instanceId }, 5000);
...
async function activateWithLemonSqueezy(licenseKey, instanceId, instanceName) {
    ...
    // did before.
    try {
        const data = await lsFetchJson(`${LEMON_SQUEEZY_API}/activate`, { license_key: licenseKey, instance_name: instanceName }, 8000);
...
async function deactivateWithLemonSqueezy(licenseKey, instanceId) {
    ...
    try {
        const data = await lsFetchJson(`${LEMON_SQUEEZY_API}/deactivate`, { license_key: licenseKey, instance_id: instanceId }, 8000);
