...
import { upsertTakeoutAlbum, linkAlbumFile, applyTakeoutSidecarMetadata, findFileIdByPathInRun, findFileIdByOriginalNameAndSize, findFileIdByOriginalNameAndDate, } from './search-database.js';
/**
 ...
 */
function takeoutTimestampToDerivedDate(unixSeconds) {
 ...
 */
function readAlbumFolder(folderPath) {
    let entries;
    try {
 ...
 * gather.
 */
export function gatherTakeoutEnrichmentFromFixResults(results) {
    const folderCache = new Map();
    const enrichment = [];
    for (const result of results) {
        if (!result.success || !result.sourcePath || !result.destPath)
            continue;
        ...
        if (!folderInfo || folderInfo.title === null)
 ...
 */
export async function gatherTakeoutEnrichmentFromZip(zipPath) {
    const stats = {
    ...
    const normaliseSeparators = (p) => p.replace(/\\/g, '/');
    const basenameOf = (e) => path.posix.basename(normaliseSeparators(e.path));
    ...
    for (const [folder, entries] of byFolder) {
        ...
        for (const e of entries) {
            ...
            if (lower.endsWith('.supplemental-metadata.json')) {
                ...
                if (entryNames.has(photoBase))
                    sidecarByPhoto.set(photoBase, e);
                continue;
            }
        ...
        for (const e of entries) {
            ...
            const candidateNames = [sidecarTitle, name].filter((s) => !!s);
            let resolvedFileId;
 ...
 */
export function importTakeoutAlbumsFromEnrichment(enrichment, 
...
runId, logger = (line) => console.log(`[Takeout] ${line}`)) {
    ...
    for (const f of enrichment.files) {
        ...
        if (existing)
            ...
            grouped.set(f.albumExternalKey, { title: f.albumTitle, files: [f] });
    }
    const summary = {
        albumsCreated: 0,
        albumsUpdated: 0,
    ...
    for (const [externalKey, group] of grouped) {
        ...
        for (const file of group.files) {
            ...
            if (applied.originalFilenameUpdated)
                originalFilenamesRecovered++;
            if (applied.captionSet)
                captionsApplied++;
        }
        const totalForAlbum = group.files.length;
        logger(`Album: ${JSON.stringify(group.title)} → ${filesLinked} files linked, ` +
    ...
    logger(`Import complete: ${summary.albumsCreated} album(s), ` +
