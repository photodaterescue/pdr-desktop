 ...
 * Date Editor, Viewer.
 */
function hardenWindowAgainstNavigation(win) {
...
function createWindow() {
    ...
    ipcMain.handle('fix:setInProgress', (_event, inProgress) => {
        ...
        for (const win of BrowserWindow.getAllWindows()) {
            if (win.isDestroyed())
                continue;
            try {
                win.webContents.send('fix:stateChanged', { inProgress });
            }
            catch { /* non-fatal */ }
        }
    });
    ...
    let lastFixProgress = null;
    ipcMain.handle('fix:broadcastProgress', (_event, payload) => {
        lastFixProgress = payload;
        for (const win of BrowserWindow.getAllWindows()) {
            if (win.isDestroyed())
                continue;
            try {
                win.webContents.send('fix:progressBroadcast', payload);
            }
            catch { /* non-fatal */ }
        }
    });
...
async function getDiskSpaceForDir(dir) {
    try {
        if (process.platform === 'win32') {
            const driveLetter = path.parse(dir).root.replace('\\', '').replace(':', '');
            if (!driveLetter)
...
async function pickPreExtractDir(zipPath, destinationPath) {
...
async function extractLargeZip(zipPath, tempDir, onProgress) {
    ...
    for (const file of directory.files) {
        ...
        try {
            ...
            if (isZlibCorruption) {
                throw Object.assign(new Error('PDR couldn\'t read this ZIP. The file appears to be corrupted, OR the connection to the source drive is unreliable (common with network drives or slow USB connections). Try copying the ZIP to a local drive on this PC first, then add it as a source.'), {
...
const pendingTakeoutEnrichments = new Map();
    ...
    try {
        ...
        if (!jpegBuffer && VIDEO_EXTS.has(ext)) {
            try {
                ...
                if (!frame)
                    frame = await extractVideoFrame(filePathLong, 0);
                if (frame) {
...
ipcMain.handle('analysis:checkExtractionsForSources', async (_event, requests) => {
    ...
    for (const req of requests) {
        ...
        const hashFor = (() => {
            try {
                return crypto.createHash('md5').update(req.path).digest('hex').substring(0, 8);
            }
            catch {
                return null;
            }
        ...
        const roots = [];
        try {
            roots.push(getCurrentPdrTempRoot());
        }
        catch { /* fall through */ }
        if (!roots.includes(PDR_TEMP_ROOT))
            roots.push(PDR_TEMP_ROOT);
        const candidates = [];
        if (hashFor) {
        ...
        try {
            if (!candidates.includes(generateTempDirName(req.path)))
                candidates.push(generateTempDirName(req.path));
        }
        catch { /* malformed */ }
        try {
            if (!candidates.includes(generateRarTempDirName(req.path)))
...
ipcMain.handle('analysis:cleanupTempDirForSource', async (_event, sourcePath) => {
    ...
    if (largeExtractInFlight && largeExtractInFlight.zipPath === sourcePath) {
        ...
        try {
            ...
            log.warn(`[Source remove] cancelAnalysis() failed (non-fatal): ${err.message}`);
        }
    ...
    const hashFor = (() => {
        try {
            return crypto.createHash('md5').update(sourcePath).digest('hex').substring(0, 8);
        }
        catch {
            return null;
        }
    ...
    try {
        if (!candidates.includes(generateTempDirName(sourcePath)))
            candidates.push(generateTempDirName(sourcePath));
    }
    catch { /* malformed */ }
    try {
        if (!candidates.includes(generateRarTempDirName(sourcePath)))
...
ipcMain.handle('analysis:sweepOrphanedTempDirsIfEmpty', async (_event, opts) => {
    ...
    for (const p of keepPaths) {
        if (typeof p !== 'string' || p.length === 0)
            continue;
        try {
            keepNames.add(path.basename(generateTempDirName(p)));
        }
        catch { /* malformed */ }
        try {
            keepNames.add(path.basename(generateRarTempDirName(p)));
        }
        catch { /* malformed */ }
    }
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        rootBytes += (await fs.promises.stat(full)).size;
                    }
                    catch { /* ignore */ }
                }
    ...
    try {
        ...
        try {
            ...
            if (enrichment) {
                ...
                console.log(`[Takeout] Gathered enrichment for ${enrichment.files.length} file(s) ` +
...
ipcMain.handle('usage:get', async (_event, licenseKey) => {
    try {
        const used = await getUsageFromWorker(licenseKey);
        return { success: true, used, limit: FREE_TRIAL_FILE_LIMIT };
    }
    catch (err) {
...
ipcMain.handle('usage:increment', async (_event, licenseKey, count) => {
    try {
...
ipcMain.handle('albums:create', async (_event, title) => {
    try {
...
ipcMain.handle('albums:rename', async (_event, albumId, newTitle) => {
    try {
...
ipcMain.handle('albums:delete', async (_event, albumId) => {
    try {
...
ipcMain.handle('albums:listPhotos', async (_event, albumId) => {
    try {
...
ipcMain.handle('albums:addPhotos', async (_event, albumId, fileIds) => {
    try {
...
ipcMain.handle('albums:removePhotos', async (_event, albumId, fileIds) => {
    try {
...
ipcMain.handle('albumGroups:listAlbumsIn', async (_event, groupId) => {
    try {
...
ipcMain.handle('albumGroups:create', async (_event, title, parentId) => {
    try {
...
ipcMain.handle('albumGroups:rename', async (_event, groupId, newTitle) => {
    try {
...
ipcMain.handle('albumGroups:delete', async (_event, groupId) => {
    try {
...
ipcMain.handle('albumGroups:move', async (_event, groupId, newParentId) => {
    try {
...
ipcMain.handle('albumGroups:removeAlbum', async (_event, albumId, groupId) => {
    try {
...
ipcMain.handle('takeout:backfillFromZip', async (_event, zipPath) => {
    try {
        ...
        console.log(`[Takeout] Backfill scan complete — ` +
...
ipcMain.handle('search:filterCounts', async (_event, query) => {
    try {
...
ipcMain.handle('library:getDriveDetails', async (_event, libraryRoot) => {
    try {
        if (!libraryRoot)
            return { success: false, error: 'libraryRoot is required' };
        ...
        if (process.platform === 'win32') {
            ...
            try {
                ...
                log.warn(`[library:getDriveDetails] CIM lookup failed for ${deviceId}:`, e.message);
            }
            ...
            try {
                ...
                log.warn(`[library:getDriveDetails] FS/Disk metadata lookup failed for ${deviceId}:`, e.message);
            }
        }
...
ipcMain.handle('library:exportDb', async () => {
    try {
        ...
        try {
            ...
            log.warn('[library:exportDb] wal_checkpoint failed (continuing with copy):', e.message);
        }
...
ipcMain.handle('library:openInExplorer', async (_event, targetPath) => {
    try {
        if (!targetPath)
...
ipcMain.on('search:viewerIndexChange', (event, index, filePath) => {
    for (const win of BrowserWindow.getAllWindows()) {
        if (win.isDestroyed())
            continue;
        if (win.webContents.id === event.sender.id)
            continue;
        try {
            win.webContents.send('search:viewerIndex', { index, filePath });
        }
        catch { }
    }
});
                ...
                try {
                    ...
                    for (const f of faces) {
                        try {
                            ...
                            fs.promises.writeFile(toLongPath(cachePath), buffer).catch(err => console.warn('[face-crop] cache write failed:', err.message));
                        }
 ...
 */
function faceCropCacheKey(filePath, boxX, boxY, boxW, boxH, size) {
    ...
    h.update(boxX.toFixed(5));
    h.update('|');
    h.update(boxY.toFixed(5));
    h.update('|');
    h.update(boxW.toFixed(5));
    h.update('|');
    h.update(boxH.toFixed(5));
    h.update('|');
...
ipcMain.handle('ai:faceCrop', async (_event, filePath, boxX, boxY, boxW, boxH, size = 96) => {
    ...
    try {
        ...
        fs.promises.writeFile(cachePathLong, buffer).catch(err => console.warn('[face-crop] cache write failed:', err.message));
...
ipcMain.handle('viewer:getRotation', async (_event, filePath) => {
    try {
        const { getUserRotation } = await import('./search-database.js');
        return { success: true, rotation: getUserRotation(filePath) };
    }
    catch (err) {
        return { success: false, error: err.message, rotation: 0 };
    }
});
ipcMain.handle('viewer:setRotation', async (_event, filePath, rotation) => {
    try {
