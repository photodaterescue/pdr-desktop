...
export function isScannerDevice(
  make: string | null | undefined,
  model: string | null | undefined,
  software: string | null | undefined = null,
): boolean {
  ...
  if (sw) {
    ...
    if (scanningApps.some(rx => rx.test(sw))) return true;
  }

  if (!mm.trim()) return false;

  // ── 2. Dedicated scanner product lines ───────────────────────────────────
  const dedicatedScannerPatterns: RegExp[] = [
    // Canon
    /\bcanoscan\b/,
    // HP
    /\bscanjet\b/,
  ...
  if (dedicatedScannerPatterns.some(rx => rx.test(mm))) return true;
  ...
  if (mfpBrandLines.some(rx => rx.test(mm))) return true;
  ...
  if (/\bcanon\b/.test(mm) && canonMfpModel.test(mm)) return true;

  // Canon imageCLASS MF-series (MF3010, MF445dw, etc.)
  if (/\bcanon\b/.test(mm) && /\bmf\d{3,4}\w*\b/.test(mm)) return true;

  // HP OfficeJet / Envy / LaserJet numeric models when the Make is HP —
  // e.g. Make='HP' / Model='8710' or 'Officejet Pro 8710'.
  if (/\bhp\b|hewlett[-\s]?packard/.test(mm) && /\b\d{4}(?:[a-z]+)?\b/.test(mm) && /(pro|plus|photo|all[-\s]?in[-\s]?one|mfp|color)/.test(mm)) return true;

  // Brother full model-prefix check (MFC-L8900CDW etc.)
  if (/\b(mfc|dcp|ads)[-\s]?[lj]?\d{3,4}\w*\b/.test(mm)) return true;

  // Fujitsu ScanSnap full model list (ix500, ix1500, ix1600, SV600, S1300i ...)
  if (/\b(ix|sv|s)\d{3,4}[a-z]?\b/.test(mm) && /fujitsu|scansnap/.test(mm)) return true;
  ...
  if (printerMakes.test(mm) && /\bseries\b/.test(mm)) return true;
