...
export const FREE_TRIAL_FILE_LIMIT = 1000;
...
export async function getUsage(licenseKey) {
    ...
    const result = await postJson('/api/usage/get', {
        key: licenseKey.trim(),
    });
 ...
 */
export async function incrementUsage(licenseKey, count) {
    if (typeof licenseKey !== 'string' || licenseKey.trim().length < 8) {
    ...
    const result = await postJson('/api/usage/increment', {
        key: licenseKey.trim(),
