...
useEffect(() => {
  ...
  // Launch-time orphan sweep. If the workspace is empty at mount, any
  // files sitting in PDR_Temp (either %TEMP%\PDR_Temp or
  ...
  // count against the 55 GB extraction cap, so without this sweep a
  // crashed extract can permanently shrink the cap until the user
  // manually deletes the temp folder.
  //
  // v2.0.11 (Terry 2026-05-24): The earlier "move sweep to startup
  // worker" attempt didn't work because the worker has no way to tell
  // which sub-folders belong to currently-persisted sources (which
  // must be preserved) vs true orphans (which can be deleted). The
  // renderer DOES have that info via localStorage. With the new
  // splash-in-its-own-window architecture, the splash stays
  // interactive even while the renderer's sweep is in flight — main
  // process is busy with fs.promises.* which yields between every
  // operation, OS message pump runs, splash drag stays smooth.
  // Fires once per app session via a ref guard.
  ...
  // v2.1 round 211 (Terry) — live count of photos added in the current
  // multi-add session, shown in the banner ("N photo(s) added") so the user
  // sees accumulation instead of a transient flash. Reset on finish/cancel.
  const [collageBgPickCount, setCollageBgPickCount] = useState(0);
  // v2.1 round 211 (Terry) — registry for the PERSISTENT per-tile marks placed
  // during a multi-add session. We track the marked tile elements (so the gold
  // outline + corner check can be torn down when the session finishes) and the
  // set of already-added paths (so clicking the same tile twice never delivers
  // a duplicate). Refs, not state: the capture-phase click handler reads/writes
  // these synchronously and they must not trigger re-renders or go stale.
  const collagePickMarksRef = useRef<Array<{ el: HTMLElement; badge: HTMLElement; prevOutline: string; prevOffset: string; prevPosition: string }>>([]);
  const collagePickPathsRef = useRef<Set<string>>(new Set());
  ...
  // v2.1 round 211 (Terry) — tear down EVERY persistent tile mark placed during
  // a multi-add session and reset the dedupe path set. Restores each tile's
  // original outline/offset/position so marks never linger into a later normal
  // browse, and removes the corner-check overlay. Safe to call repeatedly.
  const clearCollagePickMarks = useCallback(() => {
    for (const m of collagePickMarksRef.current) {
      try { m.badge.remove(); } catch { /* noop */ }
      try {
        m.el.style.outline = m.prevOutline;
        m.el.style.outlineOffset = m.prevOffset;
        m.el.style.position = m.prevPosition;
      } catch { /* noop */ }
    }
    collagePickMarksRef.current = [];
    collagePickPathsRef.current = new Set();
  }, []);
  // v2.1 round 211 (Terry) — finish a multi-add session explicitly. The photos
  // are already delivered (each click delivered with keepOpen=true), so this
  // just calls cancel() — which refocuses the collage (round 198/210) without
  // delivering anything — then clears all session state + count and removes the
  // persistent tile marks. Shared by the banner's Done button and the Esc key.
  const finishCollagePick = useCallback(() => {
    try { (window as any).pdr?.photoPick?.cancel?.(); } catch { /* noop */ }
    clearCollagePickMarks();
    setCollageBgPick(false);
    setCollageBgPickMulti(false);
    setCollageBgPickCount(0);
  }, [clearCollagePickMarks]);
  const cancelCollageBgPick = useCallback(() => {
    try { (window as any).pdr?.photoPick?.cancel?.(); } catch { /* noop */ }
    clearCollagePickMarks();        // v2.1 round 211 (Terry) — never leave marks behind
    setCollageBgPick(false);
    setCollageBgPickMulti(false);   // v2.1 round 209 (Terry) — clear the multi-add flag on exit
    setCollageBgPickCount(0);       // v2.1 round 211 (Terry) — reset the added count
  }, [clearCollagePickMarks]);
  // v2.1 round 211 (Terry) — place a PERSISTENT gold ring + top-right check on a
  // tile just added in a stay-open multi-add session, so accumulation is visible
  // (replaces the round-209 transient flash that read as "failed"). The outline
  // is robust with no positioning assumptions; the corner badge is appended
  // INTO the tile (position:absolute, top-right) so it scrolls with the tile —
  // the outline is the durable signal, the corner check is the polish.
  // Every mark is recorded in collagePickMarksRef so clearCollagePickMarks() can
  // restore the tile exactly. Works on any [data-pick-path] tile across Memories
  // / Albums / S&D and doesn't depend on purgeable Tailwind classes.
  const markPickedTile = useCallback((el: HTMLElement) => {
    try {
      ...
      // Anchor the corner badge to the tile so it sits in the TOP-RIGHT corner
      // (not the middle). Promote the tile to a positioned ancestor only if it
      // is currently static, and remember that so we can restore it on cleanup.
      const computedPos = window.getComputedStyle(el).position;
      if (computedPos === 'static') el.style.position = 'relative';
      const badge = document.createElement('div');
      badge.setAttribute('aria-hidden', 'true');
      badge.style.cssText =
        'position:absolute;z-index:30;top:6px;right:6px;display:flex;align-items:center;justify-content:center;' +
        'width:26px;height:26px;border-radius:9999px;background:var(--color-gold);color:#1f1a08;' +
        'box-shadow:0 2px 8px rgba(0,0,0,0.35);pointer-events:none;';
      badge.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      // bounce us out); the user finishes with the Done button or Esc. CTRL is
      // no longer required to add (the old "plain click finishes / CTRL stays
      // open" split was non-discoverable). Clicking the same tile twice is
      // ignored — we dedupe on the delivered path so no duplicate tiles appear.
    ...
    // v2.1 round 211 (Terry) — Esc finishes the same way as Done/Cancel:
    // cancelCollageBgPick refocuses the collage AND clears the count + tile
    // marks, so nothing lingers into a later browse.
...
const handleActivateLicense = () => {
  ...
  useEffect(() => {
    ...
    const items: TourMenuItem[] = [
      ...
      { id: 'open-ask-pdr', label: 'Ask PDR', icon: <AiSparkle className="w-4 h-4" />, onClick: () => { (window as unknown as { __pdrAskFocus?: boolean }).__pdrAskFocus = true; setActivePanel('help-support'); setTimeout(() => window.dispatchEvent(new CustomEvent('pdr:ask-pdr-focus')), 360); } },
  ...
  <>
    ...
    {collageBgPick && (
      ...
      <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[80] flex items-center gap-3 px-4 py-2.5 rounded-full bg-[var(--color-gold)] text-[#1f1a08] shadow-lg border border-[#1f1a08]/15 max-w-[90vw]">
        ...
        {/* v2.1 round 211 (Terry) — multi-add session shows a LIVE count of
            photos added so far ("N photo(s) added") plus a clear Done button;
            all clicks add (no Ctrl needed), the user finishes with Done or Esc.
            The background pick keeps its original single-click copy. */}
        {collageBgPickMulti ? (
          <div className="text-sm">
            <span className="font-semibold">
              {collageBgPickCount > 0
                ? `${collageBgPickCount} photo${collageBgPickCount === 1 ? '' : 's'} added to ${collageBgPickLabel}`
                : `Adding photos to ${collageBgPickLabel}`}
            </span>
            <span className="opacity-80"> — click photos to add them, then Done (or Esc)</span>
          </div>
        ) : (
        ...
        {/* v2.1 round 211 (Terry) — in a multi session the primary action is an
            explicit Done (reuses the Button primitive — radius/focus-ring/
            typography — themed dark-on-gold to read as the action against the
            gold banner, mirroring the Cancel pill's #1f1a08 ink). It finishes
            via finishCollagePick → cancel() (photos already delivered with
            keepOpen) + clears count + removes tile marks. The single bg pick
            keeps just the Cancel pill, unchanged. */}
        {collageBgPickMulti ? (
          <Button
            type="button"
            size="sm"
            onClick={finishCollagePick}
            className="ml-1 shrink-0 gap-1.5 bg-[#1f1a08] text-[var(--color-gold)] hover:bg-[#1f1a08]/90 border border-[#1f1a08]"
          >
            ...
            <div className="space-y-10">
              ...
              <section>
                ...
                <div className="space-y-3 text-sm text-muted-foreground leading-relaxed">
                  ...
                  <p>And once your dates are fixed, PDR becomes a full home for your photos: search and browse them, relive them in Memories, name faces in People Manager, build a Family Tree, design collages and carousels, enhance photos, record your screen, and share — all on your own hardware, nothing uploaded.</p>
                </div>
              </section>
              ...
              <section>
                ...
                <p className="text-sm text-muted-foreground mt-4 font-medium">Nothing is hidden. Nothing is guessed without being labeled.</p>
              </section>
              ...
              <section>
                ...
                <div className="space-y-3">
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-700 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">No conversion — files stay in their current format (only the extension is normalized, e.g. <span className="font-mono text-xs">.jpeg</span> → <span className="font-mono text-xs">.jpg</span>). Fastest path; the right pick for the vast majority of users. Preserves exactly what your camera or phone wrote, byte for byte.</p>
                  </div>
                  <div className="p-4 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-700 rounded-lg">
                ...
                <Accordion type="multiple" className="space-y-3">
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      <div className="space-y-6 text-sm text-muted-foreground leading-relaxed">
                        ...
                        <div className="p-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-700 rounded-lg">
                          ...
                          <p>If you've spent years curating Google Photos albums, PDR honors that work. They show up under the <span className="font-medium text-foreground">Google Photos</span> source group, ready to browse, drop into folders, or send to a Parallel Library.</p>
                        </div>
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      <div className="space-y-6 text-sm text-muted-foreground leading-relaxed">
                        ...
                        <div className="p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-700 rounded-lg">
                          ...
                          <p className="mb-2">No trustworthy clue found. PDR has used a conservative fallback (folder dates, file creation dates, last-resort best-guess) and labeled the file so you can find it later.</p>
                  ...
                  <AccordionItem value="backgroundable-analysis" className="border border-primary/40 dark:border-primary/40 rounded-lg px-4 bg-secondary/30 hover:bg-secondary/50 hover:border-primary/60 transition-all duration-200">
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      <div className="space-y-6 text-sm text-muted-foreground leading-relaxed">
                        <div className="p-4 bg-primary/5 border border-primary/10 rounded-lg">
                          <p>Source analysis on a multi-GB Takeout or a full-drive scan can take minutes (sometimes longer). You don't have to sit watching it — minimize the analysis to a small amber pill at the top of the workspace and use PDR's other views while it runs.</p>
                        </div>
                        ...
                        <div>
                          ...
                          <ul className="list-disc ml-5 space-y-1">
                            ...
                            <li>Click the minimize icon on the analysis card — the full card collapses into a slim amber pill at the top</li>
              ...
              <section className="pt-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-secondary/30 border border-border rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">When adjusting a photo in the Viewer's Enhance panel, save to a new file (the <span className="font-mono text-xs">_E</span> copy) rather than over the original, so you always keep the untouched original.</p>
                  </div>
                  <div className="p-4 bg-secondary/30 border border-border rounded-lg">
            ...
            <div className="space-y-8">
              <section>
                ...
                <div className="text-sm text-muted-foreground leading-relaxed space-y-3">
                  ...
                  <p>Beyond dates, PDR has grown into a full home for your library — browse and relive your photos in Memories, find them from the vaguest clue in Search &amp; Discovery, name faces in People Manager and build Family Trees, design Collages &amp; Carousels for print and social, capture or record your screen, and share by drag-and-drop — all on your own hardware, with nothing uploaded.</p>
                </div>
              </section>
              ...
              <section>
                ...
                <Accordion type="multiple" defaultValue={[
                  ...
                  {/* Terry 2026-06-11: v2.0.15 never shipped — its work
                      folds into v2.1, which will be the next release. The
                  ...
                  >
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      ...
                      <ul className="list-disc ml-5 space-y-1.5 text-sm text-muted-foreground v3keep">
                        ...
                        <li><strong className="text-foreground font-medium">Ask PDR</strong> &mdash; a built-in, fully offline helper in Help &amp; Support. Ask a question in your own words (&ldquo;where do my collages go?&rdquo;, &ldquo;what does _MK mean?&rdquo;, &ldquo;are my source photos safe?&rdquo;) and PDR answers straight from its own built-in help &mdash; no internet, nothing uploaded. A shortcut sits in the title-bar &ldquo;?&rdquo; menu too.</li>
                      </ul>
                      ...
                      <details className="mt-3 pt-3 border-t border-border/60 group">
                        ...
                        <li><strong className="text-foreground font-medium">Photo captions on photos AND videos</strong> &mdash; new gold caption badge on every tile that has one, edit modal with auto-focus + emoji picker, written through to the file&apos;s EXIF so captions travel with the photo. Settings &rarr; Privacy &amp; Security &ldquo;Hide captions&rdquo; lever covers the badge and the viewer caption display.</li>
                        <li><strong className="text-foreground font-medium">Needs Dates</strong> &mdash; a dedicated surface for the photos PDR couldn&apos;t date automatically. Pull up everything that still needs a date, set them by hand, and PDR writes the date through like any other Fix.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Toolbar pills are now uniform across Search &amp; Discovery, Memories &mdash; Dates, Memories &mdash; Albums, and Needs Dates</strong> &mdash; the Media filter and Display popover share the same recipe across all four surfaces (h-8, rounded, bordered, fixed minimum width with an icon on the left and a chevron on the right). The view-control row reads as one family no matter which surface you&apos;re on. Photo and video stats moved to the right side of every toolbar in the same breakdown format.</li>
                        <li><strong className="text-foreground font-medium">Sticky day banners on Memories &mdash; Dates and Memories &mdash; Albums</strong> &mdash; when you scroll, the current day&apos;s heading pins flush under the toolbar and updates as you move between days. Coordinate handling rewritten to use the scroll container as the reference frame so the banner switches days the moment a new day&apos;s photos reach the top, not after the first row passes.</li>
                        <li><strong className="text-foreground font-medium">Memories &mdash; Albums gains a &ldquo;Show Day and Date&rdquo; toggle and inline day dividers</strong> &mdash; album detail grids now optionally break by day, with a full-row divider between days and the same sticky-banner hand-off used in Memories &mdash; Dates.</li>
                        <li><strong className="text-foreground font-medium">Search &amp; Discovery preview panel hoisted into a Resizable layout</strong> &mdash; the preview now fills the toolbar-band area instead of being squashed below it. Drag the divider to size the preview vs the grid to your liking.</li>
                        <li><strong className="text-foreground font-medium">Search &amp; Discovery preview image no longer jitters when scrolling</strong> &mdash; the metadata below the photo scrolls smoothly while the photo stays pinned. Previously the photo nudged a few pixels the moment scrolling started because two separate sticky elements (the &ldquo;Details&rdquo; header and the photo block) had a hard-coded offset between them that didn&apos;t match the header&apos;s real height. Now they pin together as one group.</li>
                        <li><strong className="text-foreground font-medium">Click a tile in Search &amp; Discovery to select it</strong> &mdash; previously a plain click only highlighted the file in the preview pane; the gold selection chip never appeared unless you remembered to Ctrl-click. Now every click adds the file to your selection (and the gold banner appears), matching how Memories &mdash; Needs Dates already worked. Ctrl-click and Shift-click still expand the selection without replacing it.</li>
                        <li><strong className="text-foreground font-medium">Gold selection banner in Search &amp; Discovery</strong> &mdash; full-width band below the toolbar carrying your bulk-action controls. Left side: a clear-selection chip with the count, an &ldquo;Open N in Viewer&rdquo; gold CTA, and a More dropdown. Right side: a &ldquo;Show Preview&rdquo; placeholder occupying the future preview area as a clear cue that&apos;s where the preview will appear. Recipe shared with the Needs Dates banner so the two surfaces feel like one family.</li>
                        <li><strong className="text-foreground font-medium">Show Preview placeholder for the Ctrl-click case</strong> &mdash; if you build up a selection via Ctrl-click without ever plain-clicking, the preview panel doesn&apos;t yet have a file to show. A gold &ldquo;Show Preview&rdquo; placeholder appears in the right side of the selection banner &mdash; click it to reveal the preview panel populated with the first file in your selection.</li>
                        <li><strong className="text-foreground font-medium">Add to Parallel Library from the selection banner</strong> &mdash; pick &ldquo;Add N to Parallel Library&rdquo; from the More dropdown; the Parallel button in the Actions ribbon then pulses gold (same rate as the sidebar burger menu&apos;s lavender pulse) so you can see where the next step lives. Click the ribbon button to configure your PL with the staged selection.</li>
                        <li><strong className="text-foreground font-medium">Create new PDR album from a selection in one click</strong> &mdash; the More dropdown carries a dedicated &ldquo;Create new PDR album from N selected&rdquo; item that opens the album picker straight on the inline create-name input with your selection ready to attach. &ldquo;Add to album&hellip;&rdquo; remains the path for adding to an existing one.</li>
                        <li><strong className="text-foreground font-medium">Parallel Library promoted to the Actions ribbon</strong> &mdash; was previously buried in a More-actions ellipsis next to the result count. Now sits alongside Clear / Save / Library as a first-class export action. Selection-aware tooltip: &ldquo;Export N selected as Parallel Library&rdquo; when files are checked, &ldquo;Export all of the results as Parallel Library&rdquo; otherwise.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Video timeline scrubbing</strong> &mdash; dragging the playhead on a video in the PDR Viewer now seeks correctly instead of restarting from the beginning. The internal protocol handler now honors HTTP Range requests so the video element can fetch arbitrary byte spans for instant seeks.</li>
                        <li><strong className="text-foreground font-medium">Filmstrip no longer shows ghosted slots</strong> &mdash; the brief flash of dark placeholders while the filmstrip built itself is gone; the strip now centers on the selected thumb the moment you open a photo.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Fix Complete no longer freezes the app</strong> &mdash; the several-second hang at the end of a Fix is gone. Catalog / CSV generation moved off the main thread to a dedicated worker with per-report chunk caching.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Async geocoder load</strong> &mdash; city / country lookups initialize off the main thread so the workspace opens faster.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Discovered libraries</strong> &mdash; a new section at the bottom of the LDM that surfaces libraries PDR has Fixed photos to in the past but that aren&apos;t in your LDM list right now. Two authoritative discovery sources:
                          <ul className="list-disc ml-5 mt-1.5 space-y-1">
                            <li><strong className="text-foreground font-medium">From your Fix history</strong> (auto, fast) &mdash; reads the cumulative <code>indexed_runs</code> table (built by every Fix run since v2.0.5) and surfaces paths PDR has written to but that aren&apos;t in your saved-destinations list. Includes Fix count, last-fix timestamp, and total file count per row.</li>
                            <li><strong className="text-foreground font-medium">PDR catalog found on disk</strong> (manual, deep) &mdash; click <em>Scan drives for more</em> to walk every connected drive looking for <code>PDR_Catalogue.csv</code> files. Each catalog&apos;s contents are parsed for distinct destination paths, so a single CSV can reveal several historical libraries. The catalog file presence is the only signal used &mdash; folders without an associated catalog are never flagged, so no false positives from year-based personal organization.</li>
                          </ul>
                          Each card has one-click <strong className="text-foreground font-medium">Add to list</strong>, <strong className="text-foreground font-medium">Ignore</strong>, or <strong className="text-foreground font-medium">Open in File Explorer</strong> actions; ignored libraries stay hidden across restarts. Scan results persist across LDM close + reopen + app restart so a multi-minute scan only ever runs when you ask for it. The drive scan is depth-capped at 6 levels and skips OS-system folders, so it stays bounded even on multi-terabyte external drives.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Reset to Optimized Defaults stops touching library data</strong> &mdash; previously clicking &ldquo;Reset to Optimized Defaults&rdquo; at the bottom of Settings silently destroyed a long list of user data masquerading as &ldquo;defaults&rdquo;: your currently-configured Library Drive, your entire saved-destinations list (the LDM), your Library Planner answers, your per-camera scanner overrides (each one a curated decision you&apos;d made), your People Manager startup-days preference, your last-backup timestamp, and every &ldquo;I dismissed this advisory&rdquo; state (low-RAM, unindexed libraries, DB-backup snooze). The audit fixed both layers &mdash; the renderer-side localStorage clears (saved-destinations, planner, master-lib advisories) AND the settings-store function itself (which now honors an explicit user-data denylist). The reset now leaves all your data alone and only resets usability preferences (sounds, EXIF write toggles, AI defaults, folder structure, etc.).</li>
                        <li><strong className="text-foreground font-medium">Introducing &ldquo;PDR Photos&rdquo;</strong> &mdash; a soft brand line that appears alongside &ldquo;Photo Date Rescue&rdquo; in a few places (splash screen, Welcome screen signature, About PDR header, Help &amp; Support, Settings footer). PDR is no longer just a one-shot date-fixer &mdash; it&apos;s become a full home for your photo library, built around three principles: your photos stay on your hardware (<strong className="text-foreground font-medium">Security</strong>), nothing is uploaded or shared (<strong className="text-foreground font-medium">Privacy</strong>), and the library is yours forever (<strong className="text-foreground font-medium">Ownership</strong>). &ldquo;PDR Photos&rdquo; positions it in the same category as Apple Photos, Google Photos, and Amazon Photos &mdash; with the difference that none of those can credibly claim any of the three pillars. The full product name doesn&apos;t change, the domain doesn&apos;t change, and your installs don&apos;t change &mdash; this is a gentle introduction, not a rename.</li>
                        <li><strong className="text-foreground font-medium">Settings completely redesigned</strong> &mdash; the old horizontal-tab Settings modal had grown to 7 tabs across the top with no search; finding a specific setting was guesswork. The new Settings modal uses a Claude-style vertical sidebar on the left with icons next to each category, a search box at the top that filters by both label and keyword synonyms (typing &ldquo;refresh&rdquo; jumps to S&amp;D, &ldquo;exif&rdquo; to Workspace, &ldquo;face&rdquo; to People or AI), and all 28 settings toggles converted from checkboxes to modern toggle switches that better signal &ldquo;state&rdquo; rather than &ldquo;list selection.&rdquo; The decorative full-width &ldquo;Done&rdquo; button is gone (settings auto-save on every toggle); the X close in the top-right does the job. Footer is now a single quiet row with the version line on the left and the Reset to Optimized Defaults link on the right.</li>
                        <li><strong className="text-foreground font-medium">New Library category in Settings</strong> &mdash; library-data controls (Allow Report Removal, Allow Index Removal, Show CSV/TXT export buttons) used to be buried inside Workspace &rarr; Advanced (collapsed by default; users couldn&apos;t find them). They now have their own top-level Library tab in the Settings sidebar with a HardDrive icon. Forward-thinking layout for the additional library settings expected in future releases.</li>
                        <li><strong className="text-foreground font-medium">Master-library banner shows the folder path</strong> &mdash; the emerald &ldquo;X looks to be your master library&rdquo; banner at the top of the LDM previously displayed just the bare drive letter (e.g. <code>D:</code>) even when the recommendation was for a specific folder. It now shows the full folder path so the recommendation reads as &ldquo;PDR thinks <code>D:\1. Photos\1. PDR Library Drive</code> is your master&rdquo; rather than &ldquo;PDR thinks the whole D drive is your master.&rdquo;</li>
                        <li><strong className="text-foreground font-medium">Library Manager mini-modal groups by library</strong> &mdash; the Search &amp; Discovery &ldquo;Show Library Manager&rdquo; modal used to render one row per Fix run, so 24 Fixes to the same library showed as 24 noisy cards. Now groups by destination so each library appears once with aggregate stats (total file count across all runs, run count, last-Fix timestamp). Remove action cascades through every run for that library in one go with a confirmation that names the cascade explicitly.</li>
                        <li><strong className="text-foreground font-medium">Cleaner splash &rarr; Welcome handoff</strong> &mdash; the boot splash now stays visible until the Welcome screen is genuinely composed onto the window surface, then dismisses cleanly with no intermediate blank window between the two. Removes a brief off-white / gray flash some launches showed on slower machines. Internally: mainWindow now shows at opacity 0 first so Chromium can paint Welcome onto the visible surface before the splash steps aside, and the renderer waits for <code>document.fonts.ready</code> + paint barriers before signalling &ldquo;ready&rdquo;.</li>
                        <li><strong className="text-foreground font-medium">Add Source no longer fails with &ldquo;analysis-worker exited unexpectedly&rdquo;</strong> &mdash; in packaged installs the analysis and extract utility workers crashed at startup because their Node module-resolution cache wasn&apos;t honoring the <code>NODE_PATH</code> set by the parent. Same root cause as the v2.0.14 startup-worker fix; the same one-line <code>Module._initPaths()</code> call now runs in both workers. Prewarm-stderr is also no longer silently swallowed so any future worker startup failure shows the actual reason in <code>main.log</code>.</li>
                        <li><strong className="text-foreground font-medium">People Manager no longer opens with a stale &ldquo;PDR has stopped responding&rdquo; banner</strong> &mdash; the prewarmed PM window started heartbeat-pinging main during boot when main was busy with sidecar-snapshot, worker prewarms, and geocoder load. Two timed-out pings latched the banner permanently, so by the time you actually opened PM it was already on. PM now skips heartbeat pings while the window is hidden and fires its first real ping the moment the window becomes visible.</li>
                  ...
                  <AccordionItem value="ver-2.0.14" className="border border-border rounded-lg px-4">
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      ...
                      <ChangelogDetail>
                        ...
                        <li><strong className="text-foreground font-medium">PDR Viewer opens the image right away</strong> &mdash; the previous filmstrip-builds-first behavior stalled the image for up to 18 seconds; now the photo appears in around 100 ms and the filmstrip fills in behind it. Opening a photo from a year drilldown is also near-instant (was 60+ seconds in v2.0.13).</li>
                  ...
                  <AccordionItem value="ver-2.0.11" className="border border-border rounded-lg px-4">
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      ...
                      <ChangelogDetail>
                        ...
                        <li><strong className="text-foreground font-medium">Cleanup checks both possible temp locations</strong> &mdash; extractions normally land on the Library Drive but can fall back to a system-drive temp folder when the Library Drive is low on space. Clear Sources, orphan cleanup, and the &quot;Reclaimed X GB&quot; total now scan both locations, so what gets cleaned matches what gets reported.</li>
                  ...
                  <AccordionItem value="ver-2.0.9" className="border border-border rounded-lg px-4">
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      ...
                      <ChangelogDetail>
                        ...
                        <li><strong className="text-foreground font-medium">Developer tools access removed entirely</strong> — defense-in-depth: the F12 / Ctrl+Shift+I keyboard handler is gone in all builds. The renderer&apos;s error-listener bridge still forwards uncaught exceptions into the main-process log, so support diagnosis is unaffected.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Mouse-wheel scrolls the viewer thumbnail strip horizontally</strong> — same pattern People Manager uses. Vertical wheel input over the strip now scrolls it left-and-right. Fixed-size 80×60 tiles + instant scroll-to-active mean the strip lands in the right place from the very first frame, instead of drifting as thumbnails fill in.</li>
                        <li><strong className="text-foreground font-medium">Rapid double-clicks on a photo no longer log &quot;failed to load&quot; errors</strong> — clicking a tile while the viewer&apos;s previous open is still in flight used to log a noisy error to the diagnostic log. The duplicate click is now silently absorbed; the original open completes normally.</li>
                        ...
                        <li><strong className="text-foreground font-medium">Search &amp; Discovery: infinite scroll replaces the &quot;Load more&quot; button</strong> — the next page of results now lands automatically as you scroll near the bottom, with a small &quot;Showing N of M&quot; counter at the bottom and a final &quot;Showing all N results&quot; line when you reach the end. No more clicking through page boundaries to keep browsing.</li>
                        <li><strong className="text-foreground font-medium">Search &amp; Discovery opens faster</strong> — the filter dropdown options (Country, Camera, Lens, etc.) now load just after S&amp;D paints rather than blocking the open. The live count next to each filter option no longer recomputes 15 times per keystroke when there are no active filters yet, and the recompute is debounced harder so fast typers don&apos;t trigger a refresh between every character.</li>
                        <li><strong className="text-foreground font-medium">Search &amp; Discovery results grid is virtualised</strong> — only the visible tiles plus a small overscan window stay in the browser&apos;s memory regardless of how many infinite-scroll pages you&apos;ve loaded. A 1,000-photo result set now stays as light as a 60-photo one.</li>
                        <li><strong className="text-foreground font-medium">Memories — By Date header consolidated onto one row</strong> — the summary line, library selector, density toggle, and Jump-to-latest button now live on the same row as the By Date / Albums toggle. Frees a row&apos;s worth of vertical space for the actual photos.</li>
                  ...
                  <AccordionItem value="ver-2.0.8" className="border border-border rounded-lg px-4">
                    <AccordionTrigger className="text-foreground font-medium hover:no-underline">
                      <div className="flex items-center gap-2">
                        <span>v2.0.8</span>
                        {appVersion === '2.0.8' && (
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      ...
                      <ChangelogDetail>
                        ...
                        <li><strong className="text-foreground font-medium">Search &amp; Discovery filter overhaul — contextual counts</strong> — every dropdown filter (Country, City, Camera, Lens, etc.) now shows live counts beside each option that reflect everything else you&apos;ve selected. Tick &quot;Iceland&quot; and the City list immediately shows just the Icelandic cities and their counts. Faceted search the way it should have always worked.</li>
                        <li><strong className="text-foreground font-medium">Two-tier filter model — gates and narrowers</strong> — Confidence (Confirmed / Recovered / Marked), File Type (Photos / Videos), and Library Drive are now <em>gates</em>: all options enabled by default, and PDR shows an amber banner when any of them is cleared (because cleared gates mean zero results regardless of every other filter). Everything else is a <em>narrower</em>: empty selection = no narrowing. Removes the &quot;why are there no results?&quot; confusion entirely.</li>
                        <li><strong className="text-foreground font-medium">No-location / no-camera buckets</strong> — every location-or-camera dropdown now has a sentinel row for photos that don&apos;t have that data (&quot;No location&quot;, &quot;No camera make&quot;, etc.), so you can find your screenshots, scans, and metadata-stripped imports.</li>
                        <li><strong className="text-foreground font-medium">S&amp;D results-row cleaner</strong> — &quot;Back to Dashboard&quot; button removed (the sidebar covers it), and <em>Edit Dates</em> + <em>Create Parallel Library</em> moved into a single &quot;More actions&quot; kebab menu next to the results count. Active filters now render as inline chips between the results count and the right-side view controls, with a &quot;+N more&quot; chevron when there are too many to fit. The row reads as a calm filter-context strip instead of a wall of mixed buttons.</li>
                        <li><strong className="text-foreground font-medium">Workspace-style zoom pill in S&amp;D and Albums</strong> — the same <em>minus / percent / plus</em> zoom triplet from the Workspace is now in both the S&amp;D results bar and every Albums view header. Click the percent to reset to 50%. Ctrl+wheel scrolling over the grid also zooms.</li>
                        <li><strong className="text-foreground font-medium">Spacious / Tight density toggle in Albums</strong> — same control Memories By Date uses, now in every Albums header (All albums, per-folder, and album-detail). Picks between &quot;breathing room between photos&quot; and &quot;dense wall view&quot; and is persisted across sessions.</li>
                        <li><strong className="text-foreground font-medium">By Date library filter matches S&amp;D&apos;s style</strong> — the &quot;Library&quot; selector in Memories By Date is now a multi-select dropdown with checkboxes, identical in feel to S&amp;D&apos;s Library Drive filter. All libraries are enabled by default; uncheck the ones you don&apos;t want in the timeline.</li>
                  ...
                  <AccordionItem value="ver-2.0.0" className="border border-border rounded-lg px-4">
                    ...
                    <AccordionContent className="pt-2 pb-4">
                      ...
                      <ChangelogDetail>
                        ...
                        <li><strong className="text-foreground font-medium">Branded date picker</strong> — the native Windows date input has been replaced with a custom calendar across S&amp;D's date filter and overflow modal. Future dates are grayed out, year / month dropdowns let you jump back to 1987 in two clicks, and Today / Clear shortcuts live in the footer. Matches the rest of PDR's design language.</li>
                ...
                <div className="space-y-2">
                  <label className="flex items-center justify-between p-3 rounded-lg border border-border hover:border-primary/50 cursor-pointer transition-colors">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground">Enable deleting saved reports from history. Reports are PDR's per-Fix-run records (date, file count, results breakdown) used by Reports History and the catalog.</span>
                    </div>
                    <Switch
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex items-center justify-between p-3 rounded-lg border border-border hover:border-primary/50 cursor-pointer transition-colors">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground">Skip the "Clear sources?" prompt and remove sources automatically once a fix finishes. Off by default — keeps the prompt so you can hold onto a slow-to-analyze source for another run.</span>
                    </div>
                    <Switch
          ...
          {settingsTab === 'people' && (
            <>
              ...
              <div className="pt-4 border-t border-border">
                ...
                <p className="text-xs text-muted-foreground mb-3">Redo onboarding choices. Doesn't affect sources, reports, AI data, or your license.</p>
                <div className="flex items-center justify-between p-3 rounded-lg border border-border">
