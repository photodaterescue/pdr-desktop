...
import { getFileById, updateFileDate, getDateNeighbours, getSequentialFilenameNeighbours, getFolderMedianDate, getMedianDateForPersons, getMedianDateForGpsRadius, getFacesForFile, } from './search-database.js';
...
function extractDateFromFilename(filename) {
    ...
    for (const rx of patterns) {
        ...
        if (m) {
            const [, yy, mo, dd, hh, mi, ss] = m.map((x) => parseInt(x, 10));
...
export function getDateSuggestionsForFile(fileId) {
    ...
    try {
        ...
        const personIds = Array.from(new Set(faces.map((f) => f.person_id).filter((x) => x != null)));
...
function appendAudit(entry) {
    try {
        ...
        console.warn('[date-editor] audit log write failed:', e.message);
    }
}
export async function applyDateCorrection(opts) {
    const result = { success: true, applied: [], errors: [] };
    const settings = getSettings();
    for (const fileId of opts.fileIds) {
        const file = getFileById(fileId);
        if (!file) {
            result.errors.push({ fileId, error: 'File not found in index' });
            continue;
        }
        if (!fs.existsSync(file.file_path)) {
            result.errors.push({ fileId, error: 'File missing on disk' });
            continue;
        }
        const isoStr = typeof opts.date === 'string' ? opts.date : opts.date[fileId];
        if (!isoStr) {
            result.errors.push({ fileId, error: 'No date supplied' });
            continue;
        }
        const date = new Date(isoStr);
        if (isNaN(date.getTime())) {
            result.errors.push({ fileId, error: 'Invalid date' });
            continue;
        }
        ...
        if (opts.renameFile) {
            try {
                ...
                result.errors.push({ fileId, error: 'Rename failed: ' + e.message });
            }
        }
 ...
 */
export async function undoLastDateCorrection() {
    try {
        if (!fs.existsSync(AUDIT_LOG))
            return { success: false, error: 'No audit log' };
        const lines = fs.readFileSync(AUDIT_LOG, 'utf8').trim().split('\n').filter(Boolean);
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            ...
            if (entry.action !== 'apply' || entry.undone)
                continue;
            const file = getFileById(entry.fileId);
            if (!file)
                return { success: false, error: 'File no longer in index' };
            ...
            if (entry.renamed && entry.newPath !== entry.oldPath) {
                try {
                    ...
                    return { success: false, error: 'Failed to undo rename: ' + e.message };
                }
            }
