...
type InboundMessage = ExtractMessage | CancelMessage;
...
const parentPort = (process as unknown as {
  parentPort: {
    postMessage: (msg: ProgressMessage | DoneMessage | ErrorMessage | CancelledMessage) => void;
...
async function runExtract(zipPath: string, tempDir: string): Promise<void> {
