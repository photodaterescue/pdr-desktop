...
export function initDatabase() {
    ...
    `);
        ...
        try {
            const prefixedCount = db.prepare(`SELECT COUNT(*) AS cnt FROM indexed_files WHERE file_path LIKE '\\\\?\\%'`).get().cnt;
            if (prefixedCount > 0) {
          ...
          COMMIT;
        `);
                const remaining = db.prepare(`SELECT COUNT(*) AS cnt FROM indexed_files WHERE file_path LIKE '\\\\?\\%'`).get().cnt;
...
export async function purgeStaleIndexedFiles() {
    const database = getDb();
...
export async function purgeGhostRuns() {
    const database = getDb();
  ...
  `);
    while (queue.length > 0) {
        ...
        for (const row of rows) {
            if (row.type !== 'parent_of')
                continue;
            if (row.person_a_id !== current)
                continue; // only parent_of rows where current is parent
            const coparents = otherParentsStmt.all(row.person_b_id, current);
            for (const p of coparents) {
                if (visited.has(p.person_a_id))
                    continue;
...
function getAlbumGroupDepth(groupId) {
    const db = getDb();
    let currentId = groupId;
    let depth = 0;
    for (let i = 0; i < 16 && currentId !== null; i++) {
        const row = db.prepare(`SELECT parent_id FROM album_groups WHERE id = ?`).get(currentId);
...
function getAlbumGroupSubtreeDepth(groupId) {
    const db = getDb();
    const direct = db.prepare(`SELECT id FROM album_groups WHERE parent_id = ?`).all(groupId);
    if (direct.length === 0)
        return 0;
    let max = 0;
 ...
 */
export function listAlbumGroups() {
    const db = getDb();
    return db.prepare(`
    SELECT
 ...
 */
export function listAlbumGroupMemberships() {
    const db = getDb();
    return db.prepare(`
 ...
 */
export function listAlbumsInGroup(groupId) {
    const db = getDb();
    return db.prepare(`
    SELECT
      a.id, a.title, a.source, a.external_album_key AS externalAlbumKey,
 ...
 */
export function createUserAlbumGroup(title, parentId = null) {
    const db = getDb();
    const trimmed = title.trim() || 'New folder';
    if (parentId !== null) {
        const parent = db.prepare(`SELECT source_kind FROM album_groups WHERE id = ?`).get(parentId);
        if (!parent)
    ...
    const result = db.prepare(`INSERT INTO album_groups (title, parent_id, source_kind) VALUES (?, ?, 'user')`).run(trimmed, parentId);
...
export function renameAlbumGroup(groupId, newTitle) {
    const db = getDb();
    const trimmed = newTitle.trim();
    if (!trimmed)
        return { success: false, error: 'Folder name cannot be empty.' };
    const row = db.prepare(`SELECT source_kind FROM album_groups WHERE id = ?`).get(groupId);
    if (!row)
        return { success: false, error: 'Folder not found.' };
    if (row.source_kind === 'auto') {
        return { success: false, error: "Source folders can't be renamed." };
    }
    db.prepare(`UPDATE album_groups SET title = ?, updated_at = datetime('now') WHERE id = ?`).run(trimmed, groupId);
    return { success: true };
}
...
export function deleteAlbumGroup(groupId) {
    const db = getDb();
    const row = db.prepare(`SELECT source_kind FROM album_groups WHERE id = ?`).get(groupId);
    if (!row)
        return { success: false, error: 'Folder not found.' };
 ...
 */
export function moveAlbumGroup(groupId, newParentId) {
    const db = getDb();
    const group = db.prepare(`SELECT source_kind FROM album_groups WHERE id = ?`).get(groupId);
    if (!group)
        return { success: false, error: 'Folder not found.' };
    if (group.source_kind === 'auto')
        return { success: false, error: "Source folders can't be moved." };
    if (newParentId !== null) {
        if (newParentId === groupId)
            return { success: false, error: "Can't move a folder into itself." };
        const parent = db.prepare(`SELECT source_kind FROM album_groups WHERE id = ?`).get(newParentId);
        if (!parent)
            return { success: false, error: 'Destination folder not found.' };
        if (parent.source_kind === 'auto')
            return { success: false, error: "Can't move folders into source folders." };
        ...
        for (let i = 0; i < 16 && cursor !== null; i++) {
            if (cursor === groupId)
                return { success: false, error: "Can't move a folder into one of its own sub-folders." };
            const up = db.prepare(`SELECT parent_id FROM album_groups WHERE id = ?`).get(cursor);
    ...
    db.prepare(`UPDATE album_groups SET parent_id = ?, updated_at = datetime('now') WHERE id = ?`).run(newParentId, groupId);
    return { success: true };
}
/**
 ...
 */
export function addAlbumToGroup(albumId, groupId) {
    const db = getDb();
    const group = db.prepare(`SELECT source_kind FROM album_groups WHERE id = ?`).get(groupId);
    if (!group)
        return { success: false, inserted: false, error: 'Folder not found.' };
    if (group.source_kind === 'auto') {
        return { success: false, inserted: false, error: "Albums can't be added to source folders manually." };
    }
    const result = db.prepare(`INSERT OR IGNORE INTO album_group_memberships (album_id, group_id, is_auto) VALUES (?, ?, 0)`).run(albumId, groupId);
 ...
 */
export function removeAlbumFromGroup(albumId, groupId) {
    const db = getDb();
    const membership = db.prepare(`SELECT is_auto FROM album_group_memberships WHERE album_id = ? AND group_id = ?`).get(albumId, groupId);
    if (!membership)
